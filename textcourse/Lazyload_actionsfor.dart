...
///
/// Из видео Милка Йоханнеса
/// Если хотима в реальном времени отслеживать изменения через сеть в своем приложении
/// по получению данных и тп. То тогда строим виджеты через родительский StreamBuilder
/// , то есть поток, change in real time. Если необходимости нет такой - через
/// Future builder. И тогда изменения будут отображены при вызове ручной перерисовки
/// виджетов в приложении - setState из стандартного и тп.
///
/// Коллекции - массив, словарь, множество
/// Object in Dart - корневой тип, тут все является объектом.
/// метод map у списка(List) преобразует каждый элемент списка
/// (имясписка.map((e) => e+5).toList, последнее для преобразования в List, так
/// как тип возвращаемый мап не простой лист а MappedListIterable.
/// Отличие лист от мап-в мапе поиск по ключу(не значению, там как в листе) 
/// занимает минимум времени вне зависимости от длины мапы, а в листе поиск 
/// осуществляется по порядку с каждым элементом по очереди.
/// 
/// Hive из видео LL
/// 
